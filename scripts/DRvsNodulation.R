...
DR <- read.csv("./results/tip_DR_sep21.csv", header = TRUE, stringsAsFactors = FALSE, row.names = NULL)
...
pdf(file="./results/DR.composite.pdf", width = 24, height = 10)
ggarrange(DR.type, DR.nod, subanalysis, 
          labels = c("A", "B", "C"),
          ncol = 3, nrow = 1)
dev.off()

pdf(file="./results/comp2.pdf", width = 20, height = 7)
...
svl<-read.csv("./results/tip_DR_sep21.csv",header=TRUE,row.names=1)
log.svl <-log1p(svl)
log.svl<-setNames(log.svl[,1],rownames(log.svl))
obj<-contMap(tree,log.svl,plot=FALSE)
obj<-setMap(obj,invert=TRUE)
pdf(file="./results/reconstruction.pdf", width = 500, height = 500)
